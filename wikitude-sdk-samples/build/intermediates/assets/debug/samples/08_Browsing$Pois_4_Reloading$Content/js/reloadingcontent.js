...
var World = {
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(/*poiData,*/range) {
        ...
        var jData = []; // Harita servisinden çekilen json data kayıt değişkeni
        var url = 'http://maps.basarsoft.com.tr/BMS/yb5.ashx?f=pl&r=60&x='+World.userLocation.longitude+'&y='+World.userLocation.latitude+'&appcode=lfHNMtQezUKkUWVWMyu5OA&accid=iZJ6Uh1yB0qjgY2eYYkcaA';


		// loop through POI-information and create an AR.GeoObject (=Marker) per POI
		$(document).ready(function () {
            $.getJSON(url, function (jData){
		        for (var currentPlaceNr = 0; currentPlaceNr < jData.length; currentPlaceNr++) {

		            var dLon = jData[currentPlaceNr].Long
                    var dLat = jData[currentPlaceNr].Lat

		        	var singlePoi = {
			    	"id":/* poiData[currentPlaceNr].id*/currentPlaceNr,
			        "latitude": dLat,
				    "longitude": dLon,
				    "altitude": /*parseFloat(poiData[currentPlaceNr].altitude)*/100,
				    "title": /*poiData[currentPlaceNr].name*/ ' ',
				    "description": jData[currentPlaceNr].Name
			        };
	...
	updateRangeValues: function updateRangeValuesFn() {
        ...
        //loadPoisFromJsonDataFn(slider_value);
	...
	reloadPlaces: function reloadPlacesFn() {
		if (!World.isRequestingData) {
			if (World.userLocation) {
				...
				World.updateStatusMessage('Bilinmeyen kullanıcı konumu.', true);
			}
		}
		else {
